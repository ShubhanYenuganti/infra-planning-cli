...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && looksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'cloud-functions-pp-cli doctor' to check auth status."+
			"\n      Response: "+sanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: cloud-functions-pp-cli auth set-token <token>"+
			"\n      or: export CLOUD_FUNCTIONS_TOKEN=<your-token>"+
			"\n      Run 'cloud-functions-pp-cli doctor' to check auth status.", err))
