...
In local mode: searches locally synced data only.`,
  ...
  aws-lambda-pp-cli search "error timeout"

  # Force local search only
  aws-lambda-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  aws-lambda-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  aws-lambda-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'aws-lambda-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "2015_03_31":
				results, err = db.Search20150331(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.Search20150331(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search 2015_03_31 failed: %w", searchErr)
					}
