...
Once synced, use the 'search' command for instant full-text search.`,
  ...
  aws-lambda-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  aws-lambda-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  aws-lambda-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  aws-lambda-pp-cli sync --concurrency 8`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("aws-lambda-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/aws-lambda-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "2019_09_30":
		return db.Upsert20190930(data)
	case "2020_04_22":
		return db.Upsert20200422(data)
	case "2021_07_20":
		return db.Upsert20210720(data)
	case "2019_09_25":
		return db.Upsert20190925(data)
	case "2021_10_31":
		return db.Upsert20211031(data)
	case "2015_03_31":
		return db.Upsert20150331(data)
	case "2018_10_31":
		return db.Upsert20181031(data)
	default:
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"2015-03-31": "/2015-03-31/functions/",
		"2016-08-19": "/2016-08-19/account-settings/",
		"2018-10-31": "/2018-10-31/layers",
		"2020-04-22": "/2020-04-22/code-signing-configs/",
	}
