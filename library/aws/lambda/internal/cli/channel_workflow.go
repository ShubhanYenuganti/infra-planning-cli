...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  aws-lambda-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  aws-lambda-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"2015-03-31", "2016-08-19", "2018-10-31", "2020-04-22",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  aws-lambda-pp-cli workflow status

  # Show status as JSON
  aws-lambda-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("aws-lambda-pp-cli")
			}
