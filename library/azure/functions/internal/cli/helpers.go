...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && looksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'functions-pp-cli doctor' to check auth status."+
			"\n      Response: "+sanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: functions-pp-cli auth set-token <token>"+
			"\n      or: export WEBAPPS_CLIENT_TOKEN=<your-token>"+
			"\n      Run 'functions-pp-cli doctor' to check auth status.", err))
...
func defaultDBPath(name string) string {
	...
	return filepath.Join(home, ".infra-press", name+".db")
}
