...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  functions-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  functions-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("functions-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/functions-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  functions-pp-cli workflow status

  # Show status as JSON
  functions-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("functions-pp-cli")
			}
